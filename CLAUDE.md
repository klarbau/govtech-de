...
- **Dashboard „Dicht, aber ruhig" (shipped 2026-07-22, branch `feat/dashboard-dense`, `7aaa44a`):** Verdichtungs-Redesign nach User-Verdikt gegen die zu leere „Ein Blick, eine Antwort"-Version — User-gewählter KI-Entwurf „Variante 1" de-slopped umgesetzt (Spec `docs/specs/dashboard-dense.md`): boxlose 5er-Kennzahlen-Leiste statt KPI-Kartenband, Umzug-Glas-Hero über dem Fold (der eine Frost), 3 handbalancierte Flex-Spalten × 6 Modulkarten (Heute/Steuer | Posteingang-neu/Vorgänge | Termine/Zuletzt übermittelt), Autopilot-Fußzeile; Fristen genau einmal, kein erfundener Fortschritt, kein DSGVO-Boilerplate; alle Zusatz-Fetches parallel + ausfalltolerant, mock-backend unverändert. i18n +22/−4 Keys ×6. Gates: tsc · eslint · 868/868 · Spine 3/3 (dev) · isolierter Prod-Build grün · a11y PASS (axe 0 ×4 Kombos) · code-review REVISE→APPROVE (`7aaa44a` strikt gescoped — parallele Remote-Panel-Session hatte den Checkout mit Elterngeld-WIP kontaminiert, siehe CHANGELOG). Followups: Prod-Server-Spine-Rerun bei nächster :3000-Pause; Fremd-WIP/Branch-Kollision beim User.
- **Vorgang-Schritt-Autopilot — „das System vollzieht, der Bürger autorisiert" (shipped 2026-07-22, branch `feat/dashboard-dense`):** semantische Korrektur DIREKT auf der lebendig-Welle nach User-Verdikt „всё реально с нуля за него делается" — **kein Selbst-Abhaken mehr, nirgends** (Produktregel, Memory `no-self-report-vorgang-steps`). Alle 7 `self_assigned`-Seed-Schritte laufen jetzt als echter System-Vollzug: `starteVorgangSchritt` (einziger confirm-gated Write, Boundary-Fehler VOR Statusänderung, deterministischer Brief-Mint, idempotent, 14 Vertragstests) + `VorgangSchrittAuthDialog` (eid/consent/termin: § 18 PAuswG / Art. 6 lit. a DSGVO / Termin-Systemleistung Führerschein — ehrliche physische Ausnahme) + SSE-subscribe→reconcile (Schritt läuft LIVE: „in Arbeit"-Spinner → Erledigt-Choreo → Bestätigungs-Brief in Posteingang, NIE Bescheid). `erledigeVorgangSchritt` restlos entfernt. a11y FAIL→Fix→PASS (WCAG-2.4.3-Fokus nach Dialog-Confirm: `aria-disabled`+expliziter Post-Close-Fokus, 3 Pfade re-verifiziert). i18n −3/+14 ×6. `SEED_CONTENT_VERSION` 7→8. Gates: tsc · eslint · 875/875 · e2e 12/12 (Lifecycle 9/9 + Spine 3/3, dev) · a11y PASS · code-review REVISE→APPROVE (nur Gate-Evidenz, 0 Code-Blocker). Spec: `docs/specs/vorgang-schritt-autopilot.md`. Followups: `docs/CHANGELOG.md`.
- **Vorgang-Detail lebendig — „Erledigt-Moment statt Button + Reload" (shipped 2026-07-22, branch `feat/dashboard-dense`):** User-Verdikt „закрой все гештальты … с красивой анимацией а не просто кнопкой Schritt erledigen" — das generic Dossier (16 Seed-Vorgänge, `VorgangDetailLoader.tsx`) war visuell LG, aber die Interaktion Alt-System (Button→Toast→Skeleton-Full-Reload). Jetzt: `reconcile()` statt Reload (Seite bleibt gemountet) + gestaffelte framer-motion-Choreografie < 2 s (Node-Pop→Row-Wash→Chip→„Erledigt am"-Reveal→Segment-Fill RTL-logisch→Zähler→Banner-AnimatePresence mit Fokus-Handoff), Entrance-Stagger einmalig, `initial={false}` gegen Nach-Animation auf abgeschlossenen Akten, aria-live-Ansagen, Fehlerpfad ohne Halbzustand; Ehrlichkeit: CTA „Als erledigt markieren" + „Von Ihnen selbst zu erledigen." + „Erledigt am" (nie Autopilot-Lesart auf Eigenleistung). Seed-Gestalt: Mia „Kindergeld beantragen" (Widerspruch zur antragslos-Botschaft, Berlin↔Hamburg) → **Elterngeld § 7 BEEG** an neuer `elterngeldstelle-hamburg-eimsbuettel` + Reminder umgewidmet; `SEED_CONTENT_VERSION` 6→7. Gates: tsc · eslint · 868/868 · **Lifecycle 9/9** (NEU Test 9) · Spine 3/3 (12/12 Voll-Lauf, dev) · a11y PASS (0 Feature-Violations; Fund: pre-existing app-weiter Sonner-Toast-Kontrast light → Ticket) · +5 Keys ×6 · code-review REVISE→APPROVE (Blocker = fremdes tsconfig-Artefakt der S21-Session, per Commit-Scoping gelöst). Spec: `docs/specs/vorgang-detail-lebendig.md`. Followups: `docs/CHANGELOG.md`.
- **Termine „Übergaben" — der Termin ist die Ausnahme (shipped 2026-07-23, branch `feat/dashboard-dense`):** User-Verdikt „ai slop, придумай инновацию" gegen den Kalender-Klon → Kern-Inversion: /termine ist die Liste der **Übergaben** (Momente, wo das System die Kontrolle zurückgeben MUSS). Berechnete Thesen-Subline statt KPI-Karten (Persona-Degradation Anna/Markus/Mehmet), EIN Zeitstrahl statt Liste+Minikalender (MonthCalendar gelöscht, Tabs/Suche raus), Dossiers mit „Warum persönlich (§ 78 Abs. 3 AufenthG)"/Prep-Split system|buerger/„Ohne Anfahrt", §17-Hero-Flow erhalten; **Bündelungs-Vorschlag** (2× LEA → „In einem Besuch erledigen" via bestehende `verschiebeTermin`+`bestaetigeTerminVorschlag`, Dismiss `termin-bundling:dismissed` nach Termin-ID); **Frist = Ein-Klick-eID** über bestehenden `starteVorgangSchritt` + NEU generische Nachweis-Reminder-Resolution (7 Vertragstests); Gap-Notes mit Wiederholungs-Diät, Quiet-Rows „Läuft ohne Sie"/„Privat". Domain-Pass korrigierte 6 Rechts-/Realismus-Punkte (u. a. § 68 EStG ohne Abs., Kita-Nachweis raus, Ein-Besuch-Framing § 81 Abs. 4). `SEED_CONTENT_VERSION` 8→9. Gates: tsc · eslint · 882/882 · e2e 12/12 (dev) · a11y FAIL→Fix→**PASS** (WCAG 2.4.3 Fokus-rAF-Schleife, 1.4.10 @320) · i18n +46/−83 ×6 · code-review REVISE→**APPROVE** (Leserace, 0 Code-Blocker). Spec: `docs/specs/termine-uebergaben.md`. Followups: `docs/CHANGELOG.md`.
- **Vorgang-Detail-Restyle (shipped 2026-07-20, branch `feat/vorgang-detail-lg`):** das generic `/vorgaenge/[id]`-Template (16 Seed-Vorgänge ohne eigenes Dossier) de-slopped auf die LG-Sprache — boxless `gt-page-head` statt icon-circle-Hero, **Aktenzeichen aus Briefen statt rohem `vorgang.id`-Slug**, Progress als Segment-Balken im Schritte-Card (1-Schritt-Fix: neutral-hohl statt full-track-Amber bei „0 von 1"), Rail → EINE Details-Card (Dead-Zone auf content-armen Vorgängen weg). Review-Blocker gefixt: `FristDetailModal` (§17/§54 BMG, verifier-mandatiert) wäre app-weit verwaist → auf die Umzug-Run-Transparenz-Fußzeile relokalisiert (`run/page.tsx`, Umzug-only — shared Dossier-Bodies hätten geleakt). Gates: tsc · vitest 868/868 · lifecycle 8/8 · Spine 3/3 · a11y PASS (6 Kombos, 0 Feature-Violations) · 6-Locale-Parität (+3/−6 Keys) · code-review REVISE→APPROVE. Followups: `docs/CHANGELOG.md`.

- **Stammdaten-Datenblatt — De-Slop „Amtliches Datenblatt statt Karten-Sammelalbum" (shipped 2026-07-25, branch `feat/dashboard-dense`):** User-Verdikt „уничтожь ai slop маркеры" gegen den Green-Bento-/stammdaten (92%-Donut, Chip-Reihe, 9 uniforme Karten, 5× Fake-„Bearbeiten", Icon-Kreise, Checkmark-Wallet-Panel) → Registerauszug-These: berechnete Subline („N Angaben, geführt von M Registern · zuletzt … durch {Behörde}"), dunkle eID-Credential-Karte als Anker, EIN Datenblatt mit Hairlines + Quellen-Mikrozeilen (Melderegister/BZSt/DRV/Kasse, wohnort-gebundene `findMeldebehoerde`), boxlose Rail (Register-Status als Text, Nachweis-Zeile, ruhiges Protokoll), Partner-Label datengetrieben (Anna `ledig` ⇒ „Partner:in"). 14 Slop-Komponenten + 3 tote v1-e2e-Specs gelöscht, v3-WIP-`register-map` gerettet. NEU: 4 Text-Clipping-Gates (2× mutationsgetestet gehärtet) + 3 ICU-Render-Gates. Gates: tsc · eslint · 915/915 · a11y-Specs 25/25 · axe 0 (9 Kombos + LG-Fallback + 5 Locales) · e2e 12/12 (mit 1 isoliert-grünem Flake) · `next build` grün (isoliert) · i18n +70/−102 ×6 · a11y PASS · code-review REVISE→APPROVE. Spec: `docs/specs/stammdaten-datenblatt.md`. Followups: `docs/CHANGELOG.md`.
  - **Nachtrag 2026-07-25 (uncommitted):** eID-Karte trägt jetzt ein synthetisches Lichtbild (Lasergravur-Stil, bewusst kein Gesicht; Guillochen-Grund, Label-über-Wert-Mikrofelder, MRZ mit Dokumentcode ARD/IDD) + drei Dokument-Fakten (§ 18 PAuswG / § 78 AufenthG — eID-Funktion nur auf PA oder eAT, nie auf ausländischem Reisepass); Mobile: Datenblatt-Sektionen als Karten-Karussell, Identitätsfakten als 2-Spalten-Raster (Regel: `.m-shelf` trägt Karten, keine Textzeilen). +9 Keys ×6.
  - **Desktop-Redesign „Drei Bänder" (2026-07-25, Agenten-Pipeline, uncommitted):** nach drittem Slop-Verdikt Spec `docs/specs/stammdaten-blatt-dense.md` — Identitätsband (Karte │ Fakten-2×2 + Wallet │ Register) · Blatt zweispaltig handbalanciert mit Haarlinien-Mittelachse · Protokoll als vollbreite Tabelle; Seitenhöhe 2028→1736px, toter Quadrant strukturell weg. Gates: tsc · eslint · 915/915 · a11y-Suite 27/27 (+2 neue Assertions) · a11y-Audit PASS · code-review REVISE→erledigt (0 Code-Blocker, alle Minors/Nits angewendet). Deferred: LG=0-Serverlauf + Lighthouse (:3000-Pause). NEU-Ticket: lightningcss verwirft responsive Varianten logischer Inline-Utilities (`xl:ms-2`, `sm:ps-5`).
  - **Minimalismus-Pass (5. Iteration, User-Wahl „минимализм" nach Konzeptfrage):** Blatt ≥768 boxless (einzige Fläche = eID-Karte), Felder Label-ÜBER-Wert im Karten-Idiom ohne Zeilen-Haarlinien, small-caps-Sektionsköpfe, Mittelachse raus. Gates: 27/27 ohne Assertion-Änderung · 915/915 · Höhe @1280 1887px. NEU-Ticket: frozen `prototype-v2.css:329` `.gap-8{gap:8px}` beschattet Tailwinds `gap-8` app-weit (OnboardingWelcome, umzug/preview+start betroffen).
