...
**In progress / next:**
- [ ] **wow-backlog #1 "Kontinuierlicher Kaskaden-Moment"** (decided 2026-05-30) — stream the Umzug cascade INLINE in the assistant thread (it currently lives behind a link on `/vorgaenge/umzug/run`) + consequence receipt + Once-Only counter + Stammdaten source line + Posteingang landing. Pure presentation re-wire; highest wow×feasibility. Ranked backlog + realism guardrails: `docs/research/wow-backlog.md`.
- [ ] Onboarding/Login (DeutschlandID + EUDI) + Landing re-skin — in progress (parallel session)
- [ ] Pass-2 verticals: #2 antragsloses Kindergeld (legislated, 2027), #3 Wohngeld — after #1 lands.
