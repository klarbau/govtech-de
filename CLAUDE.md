...
**In progress / next:**
- [ ] **LG-Rollout Rest-Gates:** `next build` grün + Spine-e2e 3/3 + Kindergeld-Spine + Umzug-Resilienz (ORCH_TEST-Prod-Build) — brauchen koordinierte :3000-Pause (User). Danach kippt das code-review-REVISE auf APPROVE ohne neuen Diff. Nicht-blockierende LG-Followups in `docs/CHANGELOG.md` (u. a. `/vorgaenge/[id]` dl-Nesting WCAG 1.3.1 + dark-Kontrast `#no-next-step-title`, ~~LanguageSwitcher-Fokusring~~ (geschlossen 2026-07-10, Stammdaten-Welle), base-ui-AlertDialogs ohne data-slot, ⌘K-Pill auf `/posteingang/[id]`).
- [ ] **Stammdaten/Familie a11y-Spec-Debt (re-framed 2026-07-10):** die 6 `stammdaten-*` `test.fixme`-Specs erwarten das ALTE hero+tabs-Markup (`stammdaten-hero`, `tab-wallet` — existiert seit dem v2-Bento nicht mehr; a11y-Audit 2026-07-10 bestätigt) → brauchen **Rewrite** gegen die `v2-*`-Anker, nicht bloß un-fixme. Die 4 `redesign-familie`-Specs weiter re-evaluieren (ggf. fehlen nur testids).
