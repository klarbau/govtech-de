...
## Build log — frontend-coder (2026-06-13, mockup-fidelity rebuild)
- date: 2026-06-13
- screens implemented: Dashboard (full rebuild to provided mockup — left-column to-dos/erledigt/katalog + right rail hero/tiles/kontrolle)
- components created/modified:
  - `src/components/dashboard/DashboardView.tsx` — rebuilt render: page header (h1 + greeting h2, dropped greeting card / login-meta / prototyp-hinweis banner / since-last diff block + sort tab-chips); new `.dash-layout` (left `.dash-col` first in DOM, then right rail); added `api.getTermine()` upcoming count + `countFristenWithin14()` helper; `mapToHeuteItem` now uses aktenzeichen-or-reason subline + separate `fristDatum`; removed unused helpers (`computeLoginDays`, `formatStammdatenSubtitle`, `formatTerminSubtitle`, badge tone/label helpers, `ChipTone`).
  - `src/components/dashboard/TriumphBanner.tsx` — restyled to white `.umz-hero` card (check circle + Abgeschlossen badge, two stat columns Zeit/Stellen, full-width primary CTA); kept name/props/`aria-live`.
  - `src/components/dashboard/ErledigtFeed.tsx` — restyled as vertical timeline (green check circles + connector, right-aligned `<time>`); new optional `pending` prop renders an outline-circle Terminbestätigung row with a "Termin bestätigen" action link (meaning not color-dependent).
  - `src/components/autopilot/AutopilotKatalogTeaser.tsx` — now renders the full `.heute-card` (h2 + "Alle Lebenslagen ansehen ›" link); uniform link-as-card cells with icon-circle.lg, `.kc-meta` footer (Behörden metric + Min. gespart + chevron), subtle "demnächst" chip; dropped subtitle, dashed border, live badge, "Umzug starten" button.
  - `src/app/prototype-v2.css` — replaced dashboard block (`.dash-layout/.dash-col/.dash-greeting/.card-head-link/.heute-link/.heute-frist/.heute-chev/.dash-tiles/.stat-tile/.kontrolle-*`) and convenience block (`.umz-hero` + stat cols/divider, erledigt timeline connector + `.pending .ei-icon.outline`, `.kc-meta`, repositioned `.heute-actions`); 1-col stack ≤1024px, tiles 1-col ≤560px.
  - `src/app/(app)/dashboard/page.tsx` — unchanged.
- tests updated in lockstep:
  - `tests/e2e/spine.spec.ts` — heading assertion `'Heute zu tun'` → `'Was heute Ihre Aufmerksamkeit braucht'` (only change).
  - `tests/a11y/redesign-dashboard.spec.ts` — deleted sort-toggle test; rewrote nav-tiles test → 4 `main a.stat-tile` (hrefs `/posteingang,/posteingang,/termine,/vorgaenge`) + asserts kontrolle card links `/datenschutz`; kept ol-of-links, landmarks, reduced-motion, axe light/dark/en/ar and the `aria-labelledby="dashboard-heute-zu-tun"` wiring.
- i18n keys added (DE source — for mirror pass): dashboard.heute.{frist_bald,frist_am}; dashboard.heute.titel VALUE CHANGED to "Was heute Ihre Aufmerksamkeit braucht"; dashboard.erledigt_feed.{alle_schritte,termin_pending,termin_bestaetigen}; dashboard.triumph.{koordiniert,body,badge_abgeschlossen,zeit_label,std_value,min_value,zeit_caption,stellen_label,stellen_caption}; dashboard.tiles.{posteingang_sub,posteingang_cta,fristen_sub,fristen_cta,termine_sub,termine_cta,vorgaenge_sub,vorgaenge_cta}; dashboard.kontrolle.{title,subtitle,punkt_einwilligung,punkt_eid,punkt_rechtsgrundlage,mehr}; katalog.{lebenslagen_titel,alle_lebenslagen,behoerden_count,zeit_gespart}.
- typecheck: pass (`tsc --noEmit`, 0 errors).
- next build: green.
- deviations / improvisations:
  - Hero Zeit value: Anna's seed `receipt.geschaetzte_zeitersparnis_min` = 465 (KLASSISCH 480 − AUTOPILOT 15), so ≥120 → renders "ca. 8 Std." (NOT the brief's example "ca. 4 Std."). `receipt.behoerden_count` is the distinct-public-authorities count of confirmed steps (seed-dependent). The component math is correct; only the example numbers in the brief differ from the live seed.
  - Erledigt pending row: added an optional `pending` prop; DashboardView passes it only when `snapshot.termin_tile` exists, name resolved via behoerdenNames + termin_tile.behoerde_id, href `/termine`. Outline circle + text + action link carry meaning (no color-only).
  - Single-Behörde katalog label (Steuer, count=1) resolves `behoerden_preview[0]` → "Finanzamt Berlin Mitte/Tiergarten" (full name_de, realistic).
  - Termine tile count sources `api.getTermine()` filtered to datum >= now (graceful 0 on error), separate from the existing termin_tile (next-termin date for the sub line).
- known gaps (for code-reviewer): katalog `behoerden_label`/`untertitel`/`starten` keys are now unreferenced in the teaser but left in de.json (harmless). Did not run full a11y/e2e suites (separate passes).
- next: i18n-localizer (mirror new DE keys to en/ru/uk/ar/tr) → a11y-tester → code-reviewer.

## Build log — i18n-localizer (2026-06-13, dashboard-redesign mirror)
- date: 2026-06-13
- track: spine → FULL quality (all 5 non-DE locales human-reviewed, AR-RTL included, no `needs_review` flags left)
- locales updated: [en, ru, uk, ar, tr] (de untouched — source)
- new keys: 32 leaf keys per locale (dashboard.heute.{frist_bald,frist_am} = 2; dashboard.triumph.{koordiniert,body,badge_abgeschlossen,zeit_label,std_value,min_value,zeit_caption,stellen_label,stellen_caption} = 9; dashboard.erledigt_feed.{alle_schritte,termin_pending,termin_bestaetigen} = 3; NEW dashboard.tiles.* = 8; NEW dashboard.kontrolle.* = 6; katalog.{lebenslagen_titel,alle_lebenslagen,behoerden_count,zeit_gespart} = 4) × 5 = 160 strings
- changed keys: 1 (dashboard.heute.titel: "Heute zu tun" → "Was heute Ihre Aufmerksamkeit braucht") × 5 = 5 strings
- review-needed flags resolved: 0 added (spine = no flags); pre-existing termine.*/stammdaten.* supporting flags in _status.json left untouched (out of scope)
- terms kept in German: `Autopilot` (product term, kept in katalog.lebenslagen_titel for all 5 incl. AR "كتالوج Autopilot"); `eID` (kept Latin in dashboard.kontrolle.punkt_eid for all 5 incl. AR). No Behörden-Eigenname in this key set, so no parenthetical Latin gloss was needed — `Ämter & Institutionen` rendered as a common-noun authorities/institutions concept per locale.
- ICU/placeholders preserved verbatim: {datum} (heute.frist_am, tiles.termine_sub), {hours}/{min} (triumph.std_value/min_value), {count} (katalog.behoerden_count), {min} (triumph.zeit_label*/katalog.zeit_gespart). 'Std.'/'Min.' units localized (hrs/min · ч/мин · год/хв · ساعة/دقيقة · saat/dk).
- pronouns held: en formal you · ru Вы(cap) · uk Ви(cap) · ar فصحى formal · tr Siz. UK kept SEPARATE from RU (not auto-derived).
- known gaps: none for this key set. JSON.parse gate not runnable in this agent (no Bash) → recommend main-thread JSON.parse on all 5 + _status.json before commit (per V1.5 lesson). Structural review PASS on all 5 (balanced braces, no trailing commas, ASCII double-quote keys; erledigt_feed→tiles→kontrolle→achievement + katalog.status→lebenslagen_titel→orchestration seams verified by re-read).

## Code review - code-reviewer (2026-06-13, mockup-fidelity rebuild)
- verdict: REVISE (1 blocker). Full report: docs/reviews/2026-06-13-redesign-dashboard-code.md
- scope: dashboard-redesign footprint only (value-receipt.ts katalog fields, api.ts getAutopilotKatalog literals, DashboardView/TriumphBanner/ErledigtFeed/AutopilotKatalogTeaser, prototype-v2.css dashboard+convenience blocks, 6 locales, redesign-dashboard.spec.ts, spine.spec.ts heading rename). The uncommitted resilient-orchestration-engine workstream was OUT OF SCOPE.
- BLOCKER: tests/demo/spine-demo.spec.ts:545-546 still asserts getByRole(heading, Heute zu tun) - the heading was renamed to Was heute Ihre Aufmerksamkeit braucht. spine.spec.ts (gated) was updated; this demo:render walkthrough script was not, and will hard-fail at Scene 1 (the Loom/README video artefact is spine-tier). One-line fix + L542 comment.
- PASS otherwise: correctness (min->Std rounding, 14-day Fristen filter, upcoming-Termine count, conditional pending row, single-Behoerde katalog fallback all verified), type safety (tsc 0, no any/cast), security (no localStorage/fetch/dInnerHTML/console, no second main), conventions (strings via t(), api.* boundary), i18n (38 keys x 6 locales present, ICU 0 mismatches), a11y PASS (docs/a11y-reports/redesign-dashboard-2026-06-13.md), gated spine.spec.ts 2/2 green.
- NITs (non-blocking): orphaned sort apparatus (activeTab/getDashboardSortMode + non-ki sortTopActions branches now unreachable); orphaned i18n keys in all 6 locales (incl. katalog.titel/untertitel/behoerden_label/starten); stale dash-grid comment (L358); ErledigtFeed pending name rendered twice; tile.termine.none vs tiles.* naming; section-11 stale vs rebuild (product-architect to refresh); teaser-vs-receipt time-figure realism (45 Min teaser vs 8 Std hero for same Lebenslage - product-architect/domain to reconcile).
- status: stays spec (NOT shipped) until the demo-spec blocker is fixed and the demo render is confirmed to reach Scene 1.
