...
> **VERIFIER-VERDIKT 2026-06-28: REVISE — als STRETCH nach Pass-2 (Kindergeld/Wohngeld) bauen, NICHT jetzt.** Konzept + Recht (§1355 BGB, seit 01.05.2025) solide, aber: (1) überholt den in `demo-spine.md` benannten nächsten Bet → **Spine-Bet-Änderung = User-Eskalation, keine Verifier-Entscheidung**; (2) der zwingende Honesty-Split „Register aktualisiert: 3 Behörden" vs „Nachweise neu zu beantragen: 3 — persönliche Vorsprache" **existiert noch nicht** in `ValueReceiptCard` (nur ein `behoerden_count`) — ohne ihn re-introduziert D die „Dokumente ändern sich selbst"-Fantasie (cut-list-Grund); (3) Generalisierung von `InlineCascade` = Refactor eines shipped Spine-Bauteils (Hero-Regressionsrisiko); (4) Persona-Konflikt (Anna ist in `personas.md` verpartnert, nicht `ledig`). Voller Verdikt + 8 Pflichtkorrekturen: `docs/reviews/2026-06-28-heirats-namens-kaskade-verify.md`. Grounding/Domain: `docs/research/2026-06-28-heirats-namens-kaskade.md`. **Status: DEFERRED, pending User-Entscheidung.**
